...
def multibox(mobilenetv2, extra_layers, cfg, num_classes):
    ...
    for k, v in enumerate(mobile_source):
        ...
        loc_layers += [nn.Conv2d(channels, channels, kernel_size=3, padding=1, groups=channels),
                       nn.ReLU6(inplace=True),
                       nn.Conv2d(channels, cfg[k] * 4, kernel_size=1)]
        conf_layers += [nn.Conv2d(channels, channels, kernel_size=3, padding=1, groups=channels),
                        nn.ReLU6(inplace=True),
                        nn.Conv2d(channels, cfg[k] * num_classes, kernel_size=1)]
    for k, v in enumerate(extra_layers, 2):
        channels = v.conv[-3].out_channels
        loc_layers += [nn.Conv2d(channels, channels, kernel_size=3, padding=1, groups=channels),
                       nn.ReLU6(inplace=True),
                       nn.Conv2d(channels, cfg[k] * 4, kernel_size=1)]
        conf_layers += [nn.Conv2d(channels, channels, kernel_size=3, padding=1, groups=channels),
